...
import { getSavedDish, getUserData } from '@/utils/function'
...
const saved = () => {
  ...
  const getSavedData = async() => {
    setLoading(true)
    const result = await getSavedDish(userId)
    setDishes(result)
    setLoading(false)
  }
              ...
              <>
                {
                  ...
                  ? (
                    <View style={{ alignItems: 'center', justifyContent: 'center', gap: 16, marginTop: PixelRatio.getPixelSizeForLayoutSize(16) }}>
                      ...
                      <Text style={{ fontFamily: 'poppins-medium', fontSize: 16 * PixelRatio.getFontScale(), color: '#A0A0A0', textAlign: 'center',  }}>There's no saved dish</Text>
                    </View>
                  )
                  : (
                    <FlatList
                      refreshing={loading}
                      onRefresh={getSavedData}
                      data={dishes!}
